...
~orgnote-configuration-file-path~ - path to the configuration file. Default is `~/.config/orgnote/config.toml` (JSON is deprecated)
...
[[root]]
name = "personal"
remoteAddress = "https://org-note.com"
token = "your-token"
#+END_SRC

TOML parsing uses `tomlparse` (tree-sitter). This requires Emacs 29.1+ and the TOML grammar installed:
#+BEGIN_SRC emacs-lisp
(add-to-list
